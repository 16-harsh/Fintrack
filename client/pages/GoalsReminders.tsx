...
export default function GoalsReminders() {
  ...
  const [editingReminderId, setEditingReminderId] = useState<
    string | number | null
  >(null);
  const [editReminderForm, setEditReminderForm] =
    useState<ReminderEntry | null>(null);

  const [savingForm, setSavingForm] = useState<SavingEntry>({
    goalName: "",
    category: "General",
    targetAmount: 0,
    currentAmount: 0,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const sList: SavingEntry[] = [];
      s1.forEach((d) => sList.push({ id: d.id, ...(d.data() as any) }));
      const rList: ReminderEntry[] = [];
      s2.forEach((d) => rList.push({ id: d.id, ...(d.data() as any) }));
      setSavings(sList);
      setReminders(rList);
    })();
  }, [configured, auth]);
  ...
  async function addSaving() {
    ...
    setSavings((prev) => [
      { id: ref.id, uid: user.uid, ...savingForm },
      ...prev,
    ]);
    setSavingForm({
      goalName: "",
  ...
  async function addReminder() {
    ...
    setReminders((prev) => [
      { id: ref.id, uid: user.uid, ...reminderForm, status: "upcoming" },
      ...prev,
    ]);
    setReminderForm({
      title: "",
  ...
  async function markPaid(r: ReminderEntry) {
    ...
    setReminders((prev) =>
      prev.map((x) => (x.id === r.id ? { ...x, status: "paid" } : x)),
    );
  }
  ...
  async function snooze(r: ReminderEntry) {
    ...
    setReminders((prev) =>
      prev.map((x) =>
        x.id === r.id ? { ...x, status: "snoozed", dueDate: iso } : x,
      ),
    );
  }
  ...
  const progress = (v: number, t: number) =>
    !t ? 0 : Math.min(100, Math.round((v / t) * 100));
  ...
  async function saveEditGoal(idx: number) {
    ...
    setSavings((prev) =>
      prev.map((x) =>
        x.id === editGoalForm.id ? { ...x, ...editGoalForm } : x,
      ),
    );
    setEditingGoalId(null);
    setEditGoalForm(null);
  ...
  async function deleteGoal(s: SavingEntry, idx: number) {
    ...
    if (!user || !s.id) return;
    const db = getDb();
    await deleteDoc(doc(db, "savings", s.id));
  ...
  async function saveEditReminder(idx: number) {
    ...
    setReminders((prev) =>
      prev.map((x) =>
        x.id === editReminderForm.id ? { ...x, ...editReminderForm } : x,
      ),
    );
    setEditingReminderId(null);
    setEditReminderForm(null);
        ...
        <div className="flex flex-col gap-8">
          ...
          <section className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-xl border bg-card p-4 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="grid gap-1">
                  ...
                  <input
                    ...
                    value={savingForm.goalName}
                    onChange={(e) =>
                      setSavingForm({ ...savingForm, goalName: e.target.value })
                    }
                  />
                </div>
                <div className="grid gap-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSavingForm({ ...savingForm, category: e.target.value })
                    }
                  />
                </div>
                <div className="grid gap-1">
                ...
                <div className="grid gap-1 sm:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSavingForm({ ...savingForm, notes: e.target.value })
                    }
                  />
                </div>
                <div>
              ...
              <h3 className="mt-8 text-sm font-semibold text-foreground/70">
                Your Goals
              </h3>
                    ...
                    <div key={rowId} className="rounded-lg border p-4">
                      <div className="flex items-center justify-between gap-3">
                          ...
                          {isEditing ? (
                            <div className="grid gap-2 sm:grid-cols-2">
                              ...
                              <input
                                className="h-9 rounded-md border bg-background px-2 text-sm"
                                type="number"
                                value={Number(editGoalForm?.targetAmount) || 0}
                                onChange={(e) =>
                                  setEditGoalForm((f) => ({
                                    ...(f as any),
                                    targetAmount: Number(e.target.value),
                              ...
                              <input
                                className="h-9 rounded-md border bg-background px-2 text-sm"
                                type="number"
                                value={Number(editGoalForm?.currentAmount) || 0}
                                onChange={(e) =>
                                  setEditGoalForm((f) => ({
                                    ...(f as any),
                                    currentAmount: Number(e.target.value),
                        ...
                        <div className="text-right">
                          <p className="text-sm">
                            ...
                            {Number(
                              (isEditing
                                ? editGoalForm?.currentAmount
                                : s.currentAmount) || 0,
                            ).toLocaleString()}{" "}
                            / ₹
                            {Number(
                              (isEditing
                                ? editGoalForm?.targetAmount
                                : s.targetAmount) || 0,
                            ).toLocaleString()}
                          </p>
                        </div>
                      ...
                      <div className="mt-2 h-2 w-full overflow-hidden rounded bg-secondary">
                        <div
                          ...
                          style={{
                            width: `${progress(Number((isEditing ? editGoalForm?.currentAmount : s.currentAmount) || 0), Number((isEditing ? editGoalForm?.targetAmount : s.targetAmount) || 0))}%`,
                          }}
                        />
                      </div>
                      <div className="mt-3 flex gap-2">
                        {isEditing ? (
                          <>
                            <Button size="sm" onClick={() => saveEditGoal(idx)}>
                              Save
                            </Button>
                            <Button
                              size="sm"
                ...
                <div className="grid gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setReminderForm({
                        ...reminderForm,
                        recurring: e.target.value as any,
                      })
                    }
                  >
                    ...
                    <div key={rowId} className="rounded-lg border p-4">
                      <div className="flex items-center justify-between gap-3">
                          ...
                          {isEditing ? (
                            <div className="grid gap-2 sm:grid-cols-2">
                              ...
                              <input
                                className="h-9 rounded-md border bg-background px-2 text-sm"
                                type="number"
                                value={Number(editReminderForm?.amount) || 0}
                                onChange={(e) =>
                                  setEditReminderForm((f) => ({
                                    ...(f as any),
                                    amount: Number(e.target.value),
                              ...
                              <select
                                className="h-9 rounded-md border bg-background px-2 text-sm"
                                value={editReminderForm?.recurring || "none"}
                                onChange={(e) =>
                                  setEditReminderForm((f) => ({
                                    ...(f as any),
                                    recurring: e.target.value as any,
                              ...
                              <select
                                className="h-9 rounded-md border bg-background px-2 text-sm"
                                value={editReminderForm?.status || "upcoming"}
                                onChange={(e) =>
                                  setEditReminderForm((f) => ({
                                    ...(f as any),
                        ...
                        <p className="text-sm">
                          ₹
                          {Number(
                            (isEditing ? editReminderForm?.amount : r.amount) ||
                              0,
                          ).toLocaleString()}
                        </p>
                      </div>
                        ...
                        {isEditing ? (
                          ...
                          <>
                            <Button
                              size="sm"
                              variant="secondary"
                              onClick={() => startEditReminder(r, idx)}
                            >
                              Edit
                            </Button>
                            <Button
                            ...
                            <Button size="sm" onClick={() => snooze(r)}>
                              Snooze 7d
                            </Button>
                          </>
                        )}
